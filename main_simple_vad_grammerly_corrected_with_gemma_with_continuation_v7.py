...
app = FastAPI(title="Speech-to-Text with Feedback")
...
class EnhancedRefinementEngine:
    ...
    Returns: refined_text + speaking_feedback
    """
    def __init__(self):
        logger.info("Loading Enhanced Refinement Engine...")
        try:
            self.client = ollama.Client(host='http://localhost:11434')
            self.client.generate(model="gemma2:2b", prompt="test", options={'num_predict': 1})
            self.enabled = True
            logger.info("✓ Refinement Engine enabled")
        except Exception as e:
    ...
    async def refine_with_feedback(self, raw_text: str) -> dict:
        ...
        try:
            start = time.time()
            logger.info(f"🔧 Refining with feedback: '{raw_text}'")
...
{raw_text}"""
            ...
            logger.info(f"  Refined:  '{refined_text}'")
            logger.info(f"  Feedback: {speaking_feedback[:100]}...")
            ...
            return {
                'refined_text': raw_text,
                'speaking_feedback': f"Error generating feedback: {str(e)}",
                'raw_text': raw_text
            }

class ContinuousAudioProcessor:
    """
    Continuous transcription with feedback
    """
    def __init__(self, whisper_model, vad, refinement_engine):
        ...
        logger.info("ContinuousAudioProcessor ready")
    ...
    async def process(self) -> dict:
        """Process audio - return interim (raw only)"""
        ...
        try:
            ...
            if not has_speech:
                ...
                if len(self.accumulated_text) > 0 and self.silence_count >= self.silence_threshold:
                    return await self._finalize_with_feedback()
    ...
    async def _finalize_with_feedback(self) -> dict:
        """
        Finalize with refinement AND feedback
        Returns raw + refined + feedback
        """
        ...
        try:
            ...
            logger.info("="*60)
            logger.info(f"🎯 FINALIZING: '{raw_text}'")
            
            # Get refinement + feedback
            if self.refinement.enabled:
                result = await self.refinement.refine_with_feedback(raw_text)
                refined_text = result['refined_text']
                feedback = result['speaking_feedback']
            else:
                refined_text = raw_text
                feedback = "Refinement engine disabled."
...
class ConnectionManager:
    ...
    def __init__(self):
        ...
        self.vad = SimpleVAD(energy_threshold=0.03)
        self.refinement = EnhancedRefinementEngine()
        
        logger.info("="*60)
        logger.info(" SYSTEM READY: Raw + Refined + Feedback")
        logger.info("="*60)
    ...
    async def connect(self, websocket: WebSocket, client_id: str):
        ...
        processor = ContinuousAudioProcessor(self.whisper, self.vad, self.refinement)
            ...
            if not final_result.get('skipped') and websocket:
                try:
                    await websocket.send_json({
                        ...
                        "refined_text": final_result.get('refined_text', ''),
                        "speaking_feedback": final_result.get('speaking_feedback', ''),
                        "is_final": True,
            ...
            if should_process:
                ...
                if not result.get('skipped'):
                    if result.get('interim'):
                        ...
                        # Final: send raw + refined + feedback
                        await websocket.send_json({
                            "type": "transcription",
                        ...
                        logger.info(f"✓ Sent FINAL with feedback")
...
async def root():
    return {
        "service": "Speech-to-Text with Speaking Feedback",
        "status": "running",
        "features": ["Raw text", "Refined text", "Speaking feedback"]
    }
...
async def websocket_endpoint(websocket: WebSocket, client_id: str):
    ...
    try:
        await websocket.send_json({
            "type": "connection",
            "message": "Ready: Raw + Refined + Feedback mode"
        })
...
if __name__ == "__main__":
    import uvicorn
    print("\n" + "="*60)
    print(" SPEECH-TO-TEXT WITH SPEAKING FEEDBACK")
    print(" ⚡ Interim: Raw Whisper")
    print(" 🔧 Final: Raw + Refined + Feedback")
    print(" 📊 Learn & improve speaking skills")
    print("="*60 + "\n")
