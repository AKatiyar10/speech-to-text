...
app = FastAPI(title="Speech-to-Text with Session History")
...
class SessionHistoryManager:
    """Manages session history in JSON file"""
    ...
    def add_session(self, raw_text: str, refined_text: str, feedback: str, client_id: str):
        ...
        logger.info(f"💾 Session #{session['session_id']} saved to history")
        return session
    ...
    def get_recent_sessions(self, limit=10):
        ...
        return self.sessions[-limit:][::-1]  # Most recent first
...
class ContinuousAudioProcessor:
    """Continuous transcription with feature flags and history saving"""
    ...
    async def _finalize_with_mode(self) -> dict:
        """Finalize based on output_mode and SAVE TO HISTORY"""
        ...
        try:
            ...
            # Always generate refinement and feedback for history
            if self.refinement.enabled:
                ...
                speaking_feedback = refined_result['speaking_feedback']
            else:
                refined_text = raw_text
                speaking_feedback = "Refinement engine disabled."
            
            # 💾 SAVE TO HISTORY (always save all data)
            ...
            logger.info(f"💾 Session saved to history: #{saved_session['session_id']}")
            
            # Now return based on output mode
            ...
            logger.info(f"✅ Mode={self.output_mode.value} response prepared")
            logger.info("="*60)
...
class ConnectionManager:
    """Manages connections with feature flags and history"""
    def __init__(self, model_name="phi3:mini", history_file="session_history.json"):
        ...
        logger.info("="*60)
        logger.info(" SYSTEM READY with Session History")
        logger.info("="*60)
...
manager = ConnectionManager(model_name="gemma2:2b", history_file="session_history.json")
...
async def root():
    ...
    return {
        "service": "Speech-to-Text with Session History",
        "status": "running",
        "output_modes": {
...
if __name__ == "__main__":
    import uvicorn
    print("\n" + "="*60)
    print(" SPEECH-TO-TEXT WITH SESSION HISTORY")
    print(" 💾 All sessions saved to: session_history.json")
    print(" 🔗 API Endpoints:")
    print("   GET  /api/sessions         - All sessions")
    print("   GET  /api/sessions/recent  - Recent sessions")
    print("   GET  /api/sessions/{id}    - Specific session")
    print("   GET  /api/stats            - Statistics")
    print(" 🎙️  WebSocket modes: ?mode=raw|refined|full|all")
    print("="*60 + "\n")
